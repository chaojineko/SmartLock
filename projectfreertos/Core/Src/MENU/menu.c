...
#define MENU_MODE_TIMEOUT_MS 45000U
#define MENU_QR_MODE_TIMEOUT_MS 120000U
...
static void display_qr_unlock(void)
{
    ...
    {
        int32_t left_sec = (int32_t)(MENU_QR_MODE_TIMEOUT_MS / 1000U) -
                           (int32_t)((xTaskGetTickCount() - start_tick) / pdMS_TO_TICKS(1000));
        if (left_sec < 0)
        {
            left_sec = 0;
        ...
        if ((xTaskGetTickCount() - start_tick) >= pdMS_TO_TICKS(MENU_QR_MODE_TIMEOUT_MS))
        {
            menu_esp32_set_mode("MODE_OFF");
            current_ui_state = UI_STATE_MAIN_MENU;
            first_entry = 1;
