                                   ...
                                   uint8_t *is_unlocked)
{
...
#if UART3_CMD_DEBUG
  printf("[UART3 ProcessCmd] raw: len=%zu ", end);
  for (size_t j = 0U; j < end && j < 16U; j++)
  {
...
#if UART3_CMD_DEBUG
  printf("[UART3 ProcessCmd] after trim/upper: '%s' (len=%u)\r\n", cmd, (unsigned)strlen(cmd));
#endif
  ...
  else
  {
    ...
    if (cmd_len > 1U)
    {
      ...
      {
        char one_cmd[2] = {0};
        printf("[ESP32 CMD] split merged frame: '%s'\r\n", cmd);
        for (i = 0U; i < cmd_len; i++)
        {
          one_cmd[0] = cmd[i];
          ESP32_UART3_ProcessCmd(one_cmd, last_unlock_tick, unlock_hold_ms, is_unlocked);
        }
        return;
      }
    }
    ...
    printf("[ESP32 CMD] unknown: '%s' (len=%u)", cmd, (unsigned)strlen(cmd));
    for (size_t jj = 0U; jj < strlen(cmd) && jj < 16U; jj++)
    {
      printf(" %02X", (unsigned char)cmd[jj]);
    }
    printf("\r\n");
  }
}
...
void ESP32_UART3_Task(void *argument)
{
  ...
  const uint32_t inter_byte_timeout_ms = 300U;
  ...
  for (;;)
  {
    ...
    if (rx_status == HAL_OK)
    {
      ...
      // Incomplete frame timeout means damaged/noisy input, discard instead of executing.
      if ((idx > 0U) && ((now - last_rx_tick) > inter_byte_timeout_ms))
      {
        printf("[UART3 Timeout] discard incomplete frame idx=%u\r\n", idx);
        idx = 0;
      ...
      else
      {
        ...
        printf("[UART3 ERR] rx frame overflow\r\n");
      }
    }
