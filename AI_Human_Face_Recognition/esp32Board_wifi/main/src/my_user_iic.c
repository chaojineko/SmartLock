...
// Disable remote reset by default to avoid accidental reboot loops from noisy I2C traffic.
static bool g_i2c_remote_reset_enable = false;
...
void My_i2c_init(const QueueHandle_t key_state_i)
{
    ...
    xTaskCreate(My_IIC_Slave_Task, "My_IIC_Slave_Task", 5*1024, NULL, 5, NULL);
...
void switch_i2c_control(uint8_t REG_flag,uint8_t data)
{
    ...
    else if(REG_flag == VIRTUAL_KEY)//需要根据模式进行数据判断 虚拟按键赋值
    {
        Get_IIC_Key_massage(data);
    }
    ...
    else if(REG_flag == Model_RESET )//自动复位重启
    {
        if(data == 0x01)
        {
            if (g_i2c_remote_reset_enable)
            {
                ESP_LOGW(IICTAG, "I2C remote reset accepted");
                esp_restart();//自动复位
            }
            else
            {
                ESP_LOGW(IICTAG, "I2C remote reset ignored (disabled)");
            }
        }
...
void Get_IIC_Key_massage(uint8_t keydata)
{
