...
// RFID卡片存储（当前为RAM存储，可后续替换为Flash/EEPROM）
...
{
	// 硬件复位：根据数据手册，拉低RST至少100ns
	HAL_GPIO_WritePin(RC522_RST_PORT, RC522_RST_PIN, GPIO_PIN_RESET); // RST = 0
	osDelay(1); // 1ms > 100ns
	HAL_GPIO_WritePin(RC522_RST_PORT, RC522_RST_PIN, GPIO_PIN_SET);   // RST = 1, 退出复位
	osDelay(2); // 等待芯片稳定
	...
	Write_MFRC522(TModeReg,0x8D);				//TAuto=1，MFin高电平启动定时器，MFin低电平自动停止
	Write_MFRC522(TPrescalerReg,0x3E); 	//预分频值高8位
	Write_MFRC522(TReloadRegL,0x32);		//定时器重载值低8位
	Write_MFRC522(TReloadRegH,0x00);		//定时器重载值高8位
	Write_MFRC522(TxAutoReg,0x40); 			//100%ASK
	Write_MFRC522(ModeReg,0x3D); 				//CRC初始值0x6363
	Write_MFRC522(CommandReg,0x00);			//清空CommandReg
	AntennaOn();          							//打开天线
}
//
